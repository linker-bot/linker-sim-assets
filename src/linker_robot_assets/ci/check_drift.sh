...
#   bash src/linker_robot_assets/ci/check_drift.sh              # check all workstations
#   bash src/linker_robot_assets/ci/check_drift.sh <ws_name>    # check one workstation
#
...
if [[ ${fail} -ne 0 ]]; then
    ...
    echo "  for ws in src/linker_robot_assets/assets/workstations/*/; do python -m linker_robot_assets.composer.compose \"\$ws\"; done" >&2
